...
def call(String branch, String tag, String service, String build_num) {
    ...
    MANIFEST=sh(script: "aws ecr batch-get-image --repository-name ${REPO_NAME} --image-ids imageTag=latest --region ${Constants.AWS_REGION} --query images[].imageManifest --output text", returnStdout: true).trim()
